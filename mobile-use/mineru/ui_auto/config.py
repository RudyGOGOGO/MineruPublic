...
from mineru.ui_auto.utils.file import load_jsonc
...
class Settings(BaseSettings):
    ...
    MINITAP_BASE_URL: str = "https://platform.mineru.ai"
    ...
    GUI_OWL_BASE_URL: str = "http://127.0.0.1:8080/v1"
    GUI_OWL_MODEL: str = "MLX_GUI_Owl_8B_16bits"
...
LLMProvider = Literal["openai", "google", "openrouter", "xai", "vertexai", "mineru", "claude", "gui_owl"]
...
class LLM(BaseModel):
    ...
    def validate_provider(self, name: str):
        match self.provider:
            ...
            case "claude":
                pass  # Claude CLI uses local auth from Max subscription
            case "gui_owl":
...
def get_default_mineru_llm_config(validate: bool = True) -> LLMConfig | None:
    """
    Returns a default LLM config using the Minitap provider.
    Only returns a config if MINITAP_API_KEY is available.

    Returns:
        LLMConfig with mineru provider if API key is available, None otherwise
    """
    ...
    return LLMConfig(
        planner=LLMWithFallback(
            ...
            fallback=LLM(provider="mineru", model="meta-llama/llama-4-maverick"),
        ),
        orchestrator=LLMWithFallback(
            provider="mineru",
        ...
        contextor=LLMWithFallback(
            ...
            fallback=LLM(provider="mineru", model="meta-llama/llama-3.3-70b-instruct"),
        ),
        cortex=LLMWithFallback(
            provider="mineru",
            model="google/gemini-3-pro-preview",
            fallback=LLM(provider="mineru", model="openai/gpt-5"),
        ),
        executor=LLMWithFallback(
            provider="mineru",
            model="meta-llama/llama-3.3-70b-instruct",
            fallback=LLM(provider="mineru", model="openai/gpt-5-mini"),
        ),
        utils=LLMConfigUtils(
            outputter=LLMWithFallback(
                provider="mineru",
...
def get_gui_owl_llm_config() -> LLMConfig:
    """Returns an LLM config using GUI Owl for vision agents, Claude for text-heavy agents.

    GUI Owl (8B VLM) constraints:
    - Needs an image in every request (crashes on text-only)
    - Can't handle >30k token prompts (executor has 16 tool schemas = 158k tokens)
    - Best for: screenshot analysis with short prompts (cortex, contextor)
    - Not suited for: tool calling (executor), long-context (hopper), planning (planner)
    """
    gui_owl_fb = LLM(provider="gui_owl", model=settings.GUI_OWL_MODEL)
    claude_fb = LLM(provider="claude", model=settings.CLAUDE_MODEL)

    gui_owl_with_claude_fb = LLMWithFallback(
        provider="gui_owl",
        model=settings.GUI_OWL_MODEL,
        fallback=claude_fb,
    )
    claude_with_gui_owl_fb = LLMWithFallback(
        provider="claude",
    ...
    return LLMConfig(
        # Vision agents — GUI Owl primary (receives screenshots)
        cortex=gui_owl_with_claude_fb,
        contextor=gui_owl_with_claude_fb,
        # Text-heavy agents — Claude primary (no images, large prompts, tool calling)
        planner=claude_with_gui_owl_fb,
        orchestrator=claude_with_gui_owl_fb,
        executor=claude_with_gui_owl_fb,
        utils=LLMConfigUtils(
            outputter=claude_with_gui_owl_fb,
            hopper=claude_with_gui_owl_fb,
        ),
    )


def get_claude_llm_config() -> LLMConfig:
    """Returns an LLM config using Claude CLI for all agents."""
    claude_fb = LLM(provider="claude", model=settings.CLAUDE_MODEL)
    claude_with_fb = LLMWithFallback(
        provider="claude",
        model=settings.CLAUDE_MODEL,
        fallback=claude_fb,
    )
    return LLMConfig(
        planner=claude_with_fb,
        orchestrator=claude_with_fb,
        contextor=claude_with_fb,
        cortex=claude_with_fb,
        executor=claude_with_fb,
        utils=LLMConfigUtils(
            outputter=claude_with_fb,
            hopper=claude_with_fb,
        ),
    )
