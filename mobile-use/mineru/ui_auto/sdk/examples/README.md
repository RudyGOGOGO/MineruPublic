# ui-auto SDK Examples

Location: `src/ui_auto/sdk/examples/`

Run any example via:

- `python src/ui_auto/sdk/examples/<filename>.py`
...
This script shows the simplest way to run mineru :

- Visit https://platform.ui-auto.ai to create a task and get your API key.
...
- **LLM API Keys**: Provide necessary keys (e.g., `OPENAI_API_KEY`) in a `.env` file at repo root; see `ui_auto/config.py`.

- **Traces**: When enabled, traces are saved to a specified directory (defaulting to `./ui-auto-traces/`) and can be useful for debugging and visualization.
