...
from mineru.ui_auto.agents.outputter.outputter import outputter
from mineru.ui_auto.agents.planner.types import Subgoal
from mineru.ui_auto.clients.browserstack_client import BrowserStackClientWrapper
from mineru.ui_auto.clients.idb_client import IdbClientWrapper
from mineru.ui_auto.clients.ios_client import DeviceType, IosClientWrapper, get_ios_client
from mineru.ui_auto.clients.ui_automator_client import UIAutomatorClient
from mineru.ui_auto.clients.wda_client import WdaClientWrapper
from mineru.ui_auto.config import AgentNode, OutputConfig, record_events, settings
from mineru.ui_auto.context import (
    DeviceContext,
    DevicePlatform,
    ExecutionSetup,
    IsReplan,
...
from mineru.ui_auto.controllers.limrun_controller import (
    LimrunAndroidController,
    LimrunIosController,
)
from mineru.ui_auto.controllers.platform_specific_commands_controller import get_first_device
from mineru.ui_auto.graph.graph import get_graph
from mineru.ui_auto.graph.state import State
from mineru.ui_auto.sdk.builders.agent_config_builder import get_default_agent_config
from mineru.ui_auto.sdk.builders.task_request_builder import TaskRequestBuilder
from mineru.ui_auto.sdk.constants import DEFAULT_PROFILE_NAME
from mineru.ui_auto.sdk.services.cloud_mobile import CloudMobileService
from mineru.ui_auto.sdk.services.platform import PlatformService
from mineru.ui_auto.sdk.types.agent import AgentConfig, LimrunPlatform
...
from mineru.ui_auto.sdk.types.platform import TaskRunPlanResponse, TaskRunStatus
...
from mineru.ui_auto.utils.app_launch_utils import _handle_initial_app_launch
from mineru.ui_auto.utils.logger import get_logger
from mineru.ui_auto.utils.media import (
...
from mineru.ui_auto.utils.recorder import log_agent_thought
...
class Agent:
    ...
    def __init__(self, *, config: AgentConfig | None = None):
        ...
        self._tasks = []
        self._tmp_traces_dir = Path(tempfile.gettempdir()) / "ui-auto-traces"
    ...
    async def _install_ios_app(self, app_path: Path) -> str:
        ...
        base_url = settings.MINITAP_BASE_URL or "https://platform.mineru.ai"
    ...
    async def _run_task(
        ...
        if hasattr(self, "_limrun_controller") and self._limrun_controller is not None:
            from mineru.ui_auto.controllers.limrun_controller import LimrunAndroidController
        ...
        context = MobileUseContext(
            ...
            gui_owl_enabled=self._config.gui_owl_enabled,
            perception_mode=os.environ.get("MOBILE_USE_PERCEPTION", self._config.perception_mode),
            exploration_mode=self._config.exploration_mode,
            lessons_dir=Path(lessons_dir_str) if (lessons_dir_str := os.environ.get("MOBILE_USE_LESSONS_DIR", self._config.lessons_dir)) else None,
        )
    ...
    async def get_screenshot(self) -> Image.Image:
        ...
        if self._device_context.mobile_platform == DevicePlatform.ANDROID:
            ...
            from mineru.ui_auto.controllers.limrun_controller import LimrunIosController
