...
from mineru.ui_auto.services.gui_owl import ChatGuiOwl
...
def get_gui_owl_llm(
    ...
    """Local GUI Owl VLM model running at localhost:8080 with OpenAI-compatible API."""
    return ChatGuiOwl(
        model_name=model_name,
        temperature=temperature,
        base_url=settings.GUI_OWL_BASE_URL,
        timeout=300,  # GUI Owl prefill can take 2+ min on large screenshots
    )
...
def get_llm(
    ...
    if llm.provider == "openai":
        ...
        return ChatClaudeCLI(model_name=llm.model)
    elif llm.provider == "gui_owl":
        return get_gui_owl_llm(llm.model, temperature)
    elif llm.provider == "mineru":
        remote_tracing = False
        if ctx.execution_setup:
            remote_tracing = ctx.execution_setup.enable_remote_tracing
        return get_mineru_llm(
            trace_id=ctx.trace_id,
            remote_tracing=remote_tracing,
            model=llm.model,
            temperature=temperature,
            api_key=ctx.mineru_api_key,
            agent_name=name,
        )
    else:
