#!/bin/bash
# Create a new ui-auto SDK project
...
# Get yours at: https://platform.mineru.ai/api-keys
...
async def main() -> None:
    ...
    result = await agent.run_task(
        request=PlatformTaskRequest(
            task="your-task-name",  # Create this on platform.mineru.ai
        )
    )
    print(result)
    await agent.clean()
...
EOF
    ...
    # Download LLM config template from ui-auto repo
    echo "Downloading LLM config template..."
    curl -sL https://raw.githubusercontent.com/mineru-ai/ui-auto/main/llm-config.override.template.jsonc \
...
if [ "$MODE" = "platform" ]; then
    echo "  3. Create a task on https://platform.mineru.ai"
