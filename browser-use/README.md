...
If you have a Claude Max subscription (5x or 20x), you can use browser-use without a separate API key by routing LLM calls through the Claude Code CLI:
...
agent = Agent(
    task="Find the top 3 stories on Hacker News",
    llm=ChatClaudeCode(model='sonnet'),  # or 'opus', 'haiku'
)
await agent.run()
```

Requires the [Claude Code CLI](https://claude.ai/code) installed and authenticated (`claude` command available). Vision/screenshots are supported.
...
# Enhanced Perception (OCR + SoM)

Browser-Use supports an optional **enhanced perception mode** that adds OCR (Optical Character Recognition) and SoM (Set-of-Mark) visual grounding as a secondary observation channel alongside the existing CDP DOM pipeline.

### When to use it

The standard CDP DOM pipeline works well for most web pages. Enhanced perception fills in the gaps:
...
profile = BrowserProfile(
    perception_mode='enhanced',       # Enable OCR + SoM overlay
...
1. **OCR** (PaddleOCR) runs on each screenshot to detect text invisible to the DOM
2. **Element Merger** combines CDP elements (primary) with OCR detections, deduplicating by text + proximity
3. **SoM Overlay** draws numbered markers on the screenshot — green for clickable, gray for static, blue for OCR-only
4. The LLM receives the annotated screenshot plus a `<unified_elements>` list with coordinates for every element

The default mode (`classic`) is unchanged — enhanced perception adds ~200-500ms per step and requires `paddleocr`.
