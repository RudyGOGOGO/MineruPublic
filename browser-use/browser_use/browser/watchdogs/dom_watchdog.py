...
"""
	...
	async def on_BrowserStateRequestEvent(self, event: BrowserStateRequestEvent) -> 'BrowserStateSummary':
		...
		try:
			...
			if not_a_meaningful_website:
				...
				try:
					page_info, _device_pixel_ratio = await self._get_page_info()
				except Exception as e:
			...
			# Enhanced perception pipeline (OCR + SoM)
	...
	async def _extract_pdf_text(self, max_chars: int = 8000) -> str | None:
		...
		Uses text extraction first, falls back to OCR for scanned/image-only PDFs.
		"""
		downloads_watchdog = getattr(self.browser_session, '_downloads_watchdog', None)
	...
	async def _extract_text_from_pdf(self, pdf_path: str) -> str | None:
		"""Extract text using pypdf directly."""
		try:
			import pypdf
	...
	async def _get_page_info(self) -> tuple['PageInfo', float]:
		...
		Returns:
			Tuple of (PageInfo, device_pixel_ratio) where PageInfo contains all viewport,
			page dimensions, and scroll information, and device_pixel_ratio is the ratio
			of device pixels to CSS pixels (used by the enhanced perception pipeline).
		"""
