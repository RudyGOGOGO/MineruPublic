...
* `output_model_schema`: Pydantic model class for structured output validation. See `examples/features/custom_output.py`
...
* `initial_actions`: List of actions to run before the main task without LLM. See `examples/features/initial_actions.py`
...
* `flash_mode` (default: `False`): Fast mode that skips evaluation, next goal and thinking and only uses memory. If `flash_mode` is enabled, it overrides `use_thinking` and disables the thinking process entirely. See `examples/getting_started/05_fast_agent.py`

### System Messages

* `override_system_message`: Completely replace the default system prompt.
* `extend_system_message`: Add additional instructions to the default system prompt. See `examples/features/custom_system_prompt.py`
...
* `sensitive_data`: Dictionary of sensitive data to handle carefully. See `examples/features/sensitive_data.py`
...
See all helper methods in `browser_use/agent/views.py` (AgentHistoryList class).

## Structured Output

For structured output, use the `output_model_schema` parameter with a Pydantic model. See `examples/features/custom_output.py`.
...
* get 2fa codes
* send emails
* Playwright integration (see `examples/browser/playwright_integration.py`)
* ...
...
Here is the source code (`browser_use/tools/service.py`) for the default tools:
