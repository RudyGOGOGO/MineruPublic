...
Uses `cdp-use` for typed CDP protocol access. All CDP client management lives in `browser_use/browser/session.py`.
...
We use a thin wrapper around CDP called cdp-use. cdp-use only provides shallow typed interfaces for the websocket calls, all CDP client and session management + other CDP helpers still live in browser_use/browser/session.py.
...
## Example Scripts — Always Use the Agent System

- **Example scripts in `examples/` MUST use the full `Agent` class** to drive browser interactions. They exist to verify that the agentic system works end-to-end, not to test individual components in isolation.
- **NEVER write example scripts that call internal pipeline functions directly** (e.g. `OCREngine.get().detect()`, `_cdp_to_unified_elements()`, `merge_elements()`, `render_som_overlay()`, `enhance_observation()`, `get_browser_state_summary()`). If the agent system can't do something through its normal loop, that's a real bug to fix in the system — not something to work around in an example script.
- **The pattern for every example script is**: create a `BrowserProfile`, create a `BrowserSession`, create an `Agent` with a task, call `agent.run()`, print `result.final_result()`. That's it.
- Example scripts test the system as a user would experience it. If an example needs hardcoded workarounds to function (e.g. manually calling OCR, redirecting to a different viewer, extracting text outside the agent loop), that signals a gap in the agentic system that should be fixed in the library code, not papered over in examples.
