...
def _get_allowed_paths() -> list[str]:
    """Get the list of allowed host paths from sandbox config."""
    try:
        from deerflow.config import get_app_config

        config = get_app_config()
        return config.sandbox.allowed_paths
    except Exception:
        return []


def _is_allowed_path(path: str) -> bool:
    """Check if a path is under one of the configured allowed_paths."""
    allowed = _get_allowed_paths()
    if not allowed:
        return False
    resolved = str(Path(path).resolve())
    return any(resolved == a or resolved.startswith(a.rstrip("/") + "/") for a in allowed)
...
def validate_local_tool_path(path: str, thread_data: ThreadDataState | None, *, read_only: bool = False) -> None:
    ...
    # Paths explicitly allowed via sandbox.allowed_paths
...
def _validate_resolved_user_data_path(resolved: Path, thread_data: ThreadDataState) -> None:
    ...
    and is not under a configured allowed_paths entry.
    """
    allowed_roots = [
        Path(p).resolve()
        for p in (
    ...
    # Also include sandbox.allowed_paths as allowed roots
    for allowed in _get_allowed_paths():
        allowed_roots.append(Path(allowed).resolve())
...
def validate_local_bash_command_paths(command: str, thread_data: ThreadDataState | None) -> None:
    ...
    for absolute_path in _ABSOLUTE_PATH_PATTERN.findall(command):
        ...
        # Allow paths explicitly configured in sandbox.allowed_paths
        if _is_allowed_path(absolute_path):
            _reject_path_traversal(absolute_path)
            continue
...
def sandbox_from_runtime(runtime: ToolRuntime[ContextT, ThreadState] | None = None) -> Sandbox:
    ...
    sandbox_mode = _get_sandbox_mode(runtime)
    sandbox = get_sandbox_provider(name=sandbox_mode).get(sandbox_id)
    if sandbox is None:
...
def _get_sandbox_mode(runtime: ToolRuntime[ContextT, ThreadState] | None) -> str | None:
    """Extract sandbox_mode from the runtime configurable context."""
    if runtime is None or runtime.context is None:
        return None
    cfg = runtime.context.get("configurable", {})
    if isinstance(cfg, dict):
        return cfg.get("sandbox_mode")
    return getattr(cfg, "sandbox_mode", None)
...
def ensure_sandbox_initialized(runtime: ToolRuntime[ContextT, ThreadState] | None = None) -> Sandbox:
    ...
    provider = get_sandbox_provider(name=sandbox_mode)
    ...
    try:
        ...
        if is_local_sandbox(runtime):
            ...
            if _is_skills_path(path):
                ...
                pass  # Use the host path directly
            else:
