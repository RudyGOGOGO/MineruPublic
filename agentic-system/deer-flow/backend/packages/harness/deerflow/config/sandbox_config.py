...
class SandboxConfig(BaseModel):
    ...
    Common options:
        ...
        display_name: Human-readable name for the sandbox (optional, used in frontend)
    ...
    display_name: str | None = Field(
        default=None,
        description="Human-readable name for display in the frontend",
    )
    ...
    allowed_paths: list[str] = Field(
        default_factory=list,
        description="List of absolute host paths that the agent is allowed to access in local sandbox mode. "
        "Paths are matched as prefixes, so /Users/foo/project allows access to all files under that directory.",
    )
