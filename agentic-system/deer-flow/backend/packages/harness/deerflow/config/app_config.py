...
class AppConfig(BaseModel):
    ...
    sandbox: SandboxConfig = Field(description="Default sandbox configuration")
    sandboxes: dict[str, SandboxConfig] = Field(default_factory=dict, description="Named sandbox configurations for runtime switching")
    ...
    def get_sandbox_config(self, name: str | None = None) -> SandboxConfig:
        """Get sandbox config by name.

        If name is None or not found in named sandboxes, returns the default sandbox config.

        Args:
            name: The name of the sandbox to get (e.g. "local", "container").

        Returns:
            The sandbox config.
        """
        if name and name in self.sandboxes:
            return self.sandboxes[name]
        return self.sandbox

    def get_available_sandboxes(self) -> dict[str, SandboxConfig]:
        """Get all available sandbox configurations.

        Returns a dict of sandbox name -> config. If no named sandboxes are configured,
        returns just the default sandbox under the key "default".

        Returns:
            Dictionary of sandbox name to config.
        """
        if self.sandboxes:
            return dict(self.sandboxes)
        return {"default": self.sandbox}
