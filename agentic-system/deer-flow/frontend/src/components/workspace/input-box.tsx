...
import { useSandboxes } from "@/core/sandboxes/hooks";
...
export function InputBox({
  ...
  const selectedSandbox = useMemo(() => {
    ...
    return (
      sandboxes.find((s) => s.name === context.sandbox_mode) ?? sandboxes[0]
    );
  }, [context.sandbox_mode, sandboxes]);

  // Auto-select first sandbox when sandboxes load
  useEffect(() => {
    if (sandboxes.length > 0 && !context.sandbox_mode) {
      onContextChange?.({
        ...context,
        sandbox_mode: sandboxes[0]!.name,
      });
    }
  }, [sandboxes, context, onContextChange]);
            ...
            {sandboxes.length > 1 && (
              <PromptInputActionMenu>
                <PromptInputActionMenuTrigger className="gap-1! px-2!">
                  ...
                  <div className="text-xs font-normal">
                    {selectedSandbox?.display_name ?? selectedSandbox?.name}
                  </div>
                </PromptInputActionMenuTrigger>
                <PromptInputActionMenuContent className="w-70">
                  <DropdownMenuGroup>
                    ...
                    <PromptInputActionMenu>
                      {sandboxes.map((s) => (
                        <PromptInputActionMenuItem
                          key={s.name}
                          className={cn(
                            context.sandbox_mode === s.name
                              ? "text-accent-foreground"
                              : "text-muted-foreground/65",
                          )}
                          onSelect={() => handleSandboxSelect(s.name)}
                        >
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-1 font-bold">
                              {s.name === "local" ? (
                            ...
                            <div className="pl-7 text-xs">
                              {s.name === "local"
                                ? t.inputBox.sandboxLocalDescription
                                : t.inputBox.sandboxContainerDescription}
                            </div>
                          </div>
                          {context.sandbox_mode === s.name ? (
                            <CheckIcon className="ml-auto size-4" />
